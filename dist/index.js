...
Better Auth is designed to be the most comprehensive auth solution while remaining simple to use and extend.`;
        ...
        const doc = BETTER_AUTH_DOCS[topic];
        if (!doc) {
...
export default defineEventHandler((event) => {
    ...
    async generateClientSetup(args) {
...
if (session.data) {
    ...
    async generateDatabaseSchema(args) {
...
export async function loader({ request }: LoaderFunctionArgs) {
    ...
    return json({ user: session.user });
        ...
        return {
            content: [{
                    type: "text",
                    text: `# ${useCase.split('-').map((w) => w.charAt(0).toUpperCase() + w.slice(1)).join(' ')} Example\n\n${example}`
                }]
        };
    }
    async troubleshootIssue(args) {
...
4. Check plugin documentation for specific setup requirements`
        ...
        const matchedSolution = commonIssues.find(item => item.keywords.some(keyword => lowerIssue.includes(keyword)));
        if (matchedSolution) {
            return {
                content: [{
                        type: "text",
