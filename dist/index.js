...
class BetterAuthServer {
    ...
    BETTER_AUTH_DOCS_URL = "https://www.better-auth.com";
    BETTER_AUTH_GITHUB_URL = "https://github.com/better-auth/better-auth";
     ...
     */
    async fetchFeatureBasicInfo(featureName, category) {
     ...
     */
    async handleGetFeatureDetails(args) {
     ...
     * Fetch detailed feature information
     */
    async fetchFeatureDetails(featureName) {
     ...
     */
    findFeatureCategory(featureName) {
     ...
     */
    async handleGetIntegrationGuide(args) {
     ...
     */
    async fetchIntegrationGuide(framework) {
        ...
        try {
            ...
            throw new McpError(ErrorCode.InvalidParams, `Integration guide for "${framework}" not found`);
        }
    }
    /**
     * Handle the get_auth_examples tool request
     */
    async handleGetAuthExamples(args) {
        const featureName = this.validateFeatureName(args);
     ...
     */
    async fetchAuthExamples(featureName, framework) {
        const examples = [];
     ...
     */
    async handleSearchBetterAuth(args) {
     ...
     * Validation helpers
     */
    validateFeatureName(args) {
        if (!args?.featureName || typeof args.featureName !== "string") {
            throw new McpError(ErrorCode.InvalidParams, "Feature name is required and must be a string");
        }
        return args.featureName.toLowerCase();
    }
    validateFramework(args) {
        if (!args?.framework || typeof args.framework !== "string") {
            throw new McpError(ErrorCode.InvalidParams, "Framework name is required and must be a string");
        }
     ...
     */
    async ensureFeaturesListLoaded() {
        if (!this.featuresListCache) {
            await this.loadFeaturesFromDocs();
        }
        if (!this.featuresListCache) {
            throw new McpError(ErrorCode.InternalError, "Failed to load Better Auth features list");
        }
    }
    /**
     * Run the server
