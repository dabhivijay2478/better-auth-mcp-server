...
async function initResources(server) {
    try {
        ...
        server.registerResource("better_auth_complete_docs", "better-auth://complete-documentation", {
...
async function initTools(server) {
    try {
                ...
                return {
                    content: [{
                            ...
                            text: JSON.stringify({
                                ...
                                categories: {
                                    basic: Object.keys(AUTH_PROVIDERS).filter(key => AUTH_PROVIDERS[key].category === 'basic'),
                                    social: Object.keys(AUTH_PROVIDERS).filter(key => AUTH_PROVIDERS[key].category === 'social')
                                },
                ...
                return {
                    content: [{
                            ...
                            text: JSON.stringify({
                                ...
                                withMigrationSupport: Object.keys(DATABASE_ADAPTERS).filter(key => DATABASE_ADAPTERS[key].migrationSupport),
                ...
                return {
                    content: [{
                            ...
                            text: JSON.stringify({
                                ...
                                categories: {
                                    security: Object.keys(PLUGINS).filter(key => PLUGINS[key].category === 'security'),
                                    basic: Object.keys(PLUGINS).filter(key => PLUGINS[key].category === 'basic'),
                                    enterprise: Object.keys(PLUGINS).filter(key => PLUGINS[key].category === 'enterprise'),
                                    social: Object.keys(PLUGINS).filter(key => PLUGINS[key].category === 'social'),
                                    session: Object.keys(PLUGINS).filter(key => PLUGINS[key].category === 'session'),
                                    api: Object.keys(PLUGINS).filter(key => PLUGINS[key].category === 'api')
                                },
                                withClientSetup: Object.keys(PLUGINS).filter(key => PLUGINS[key].clientSetup),
        ...
        server.registerTool("generate_auth_config", {
            ...
            try {
                // Validate inputs
                const invalidProviders = providers.filter(p => !AUTH_PROVIDERS[p]);
                const invalidPlugins = plugins.filter(p => !PLUGINS[p]);
                ...
                const adapterImport = DATABASE_ADAPTERS[adapter].requiredPackages.length > 0 ?
                    `import { ${DATABASE_ADAPTERS[adapter].requiredPackages.join(', ')} } from "${DATABASE_ADAPTERS[adapter].requiredPackages[0]}";` : '';
                // Generate config
...
export const auth = betterAuth({
  database: ${DATABASE_ADAPTERS[adapter].codeExample.split('database: ')[1].split(',')[0]},
...
${plugins.filter(p => PLUGINS[p].clientSetup).length > 0 ?
                    `import { ${plugins.filter(p => PLUGINS[p].clientSetup).map(p => `${p}Client`).join(', ')} } from "better-auth/client/plugins";` : ''}

export const authClient = createAuthClient({
  baseURL: process.env.NEXT_PUBLIC_BETTER_AUTH_URL || "http://localhost:3000",
  ${plugins.filter(p => PLUGINS[p].clientSetup).length > 0 ? `plugins: [
    ${plugins.filter(p => PLUGINS[p].clientSetup).map(p => `${p}Client()`).join(',\n    ')}
  ]` : ''}
});` : '';
                ...
                const allEnvVars = [
                    'BETTER_AUTH_SECRET',
                    'BETTER_AUTH_URL',
                    ...providers.flatMap(p => AUTH_PROVIDERS[p]?.requiredEnvVars || [])
                ];
                // Generate framework-specific handler if specified
                const frameworkHandler = framework && FRAMEWORK_INTEGRATIONS[framework] ?
                    FRAMEWORK_INTEGRATIONS[framework].codeExample : '';
                return {
                    content: [{
                            type: "text",
                            text: JSON.stringify({
                                ...
                                requiredPackages: [
                                    "better-auth",
                                    ...DATABASE_ADAPTERS[adapter].requiredPackages,
                                    ...plugins.flatMap(p => PLUGINS[p]?.requiredPackages || [])
                                ],
        ...
        server.registerTool("get_framework_integration", {
            ...
            try {
                const integration = FRAMEWORK_INTEGRATIONS[framework];
                if (!integration) {
        ...
        server.registerTool("get_mcp_plugin_config", {
  ...
  plugins: [
    mcp({
      scope: ["profile", "email"]
        ...
        server.registerTool("get_mcp_session_handler", {
...
export default withMcpAuth(
  ...
  auth
        ...
        server.registerTool("get_oauth_discovery_metadata", {
...
export const GET = oAuthDiscoveryMetadata(auth);`,
        ...
        server.registerTool("get_mcp_session_api", {
...
const handler = async (req: Request) => {
  ...
  return new Response(JSON.stringify({
    ...
    authenticated: true
        ...
        server.registerTool("get_multi_session_plugin_config", {
...
export const authClient = createAuthClient({
  plugins: [
    multiSessionClient()
        ...
        server.registerTool("get_email_verification_config", {
...
await authClient.verifyEmail({
  query: { token: "verification_token" }
        ...
        server.registerTool("get_password_reset_config", {
...
await authClient.resetPassword({
  ...
  token: "reset_token"
        ...
        server.registerTool("get_session_management_config", {
...
const session = await auth.api.getSession({
  headers: request.headers
        ...
        server.registerTool("get_database_schema_info", {
  ...
  session: {
    ...
    fields: {
      userId: "user_id"
        ...
        server.registerTool("get_hooks_config", {
...
await authClient.signIn.email({ email, password })`
                ...
                const migration = migrations[from_solution];
                if (!migration) {
        ...
        server.registerTool("get_quick_start_guide", {
            ...
            try {
                const quickStart = {
                    ...
                    installation: [
                        "npm install better-auth",
                        ...DATABASE_ADAPTERS[database]?.requiredPackages.map(pkg => `npm install ${pkg}`) || []
                    ],
...
import { betterAuth } from "better-auth";
${DATABASE_ADAPTERS[database]?.codeExample.split('\n')[0] || ''}

export const auth = betterAuth({
  database: ${DATABASE_ADAPTERS[database]?.codeExample.split('database: ')[1].split(',')[0] || 'undefined'},
  ...
  ${auth_methods.filter(m => m !== 'email-password').length > 0 ? `socialProviders: {
    ${auth_methods.filter(m => m !== 'email-password').map(provider => `${provider}: {
      ...
      clientSecret: process.env.${provider.toUpperCase()}_CLIENT_SECRET!
                    ...
                    frameworkSetup: FRAMEWORK_INTEGRATIONS[framework]?.codeExample || "Framework setup not available",
