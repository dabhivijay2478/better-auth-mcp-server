...
const SERVER_VERSION = "1.0.2";
    ...
    let LLM_DOCS_CACHE: LoadedDoc[] | null = null;

    function normalizeTopic(text: string): string {
      return text.toLowerCase().replace(/[^a-z0-9]+/g, " ").trim();
    }

function splitIntoParasWithLineRanges(text: string): Array<{ startLine: number; endLine: number; paragraph: string }> {
      const lines = text.split(/\r?\n/);
      const paragraphs: Array<{ startLine: number; endLine: number; paragraph: string }> = [];
      let buffer: string[] = [];
      let paraStart = 1;
      for (let i = 0; i < lines.length; i++) {
        ...
        if (line.trim() === "") {
          ...
          if (buffer.length === 0) paraStart = i + 1;
          buffer.push(line);
        }
      }
  if (buffer.length > 0) paragraphs.push({ startLine: paraStart, endLine: lines.length, paragraph: buffer.join("\n") });
      return paragraphs;
    }

    function loadLLMDataset(): LoadedDoc[] {
      if (LLM_DOCS_CACHE) return LLM_DOCS_CACHE;
      if (!fs.existsSync(LLM_DIR)) return (LLM_DOCS_CACHE = []);
  ...
  LLM_DOCS_CACHE = files.map((f) => {
    ...
    function pickCandidateFiles(allDocs: LoadedDoc[], topic?: string): LoadedDoc[] {
      if (!topic || topic.trim() === "") return allDocs;
    ...
    ["reference", ["api", "options", "reference"]],
      ...
      if (t.includes(canon) || synonyms.some((s) => t.includes(s))) {
        if (n.includes(canon) || synonyms.some((s) => n.includes(s))) score += 2;
          }
        }
        return score;
      }
  ...
  const kept = scored.filter((x) => x.s >= Math.max(1, max - 1)).map((x) => x.d);
    ...
    function scoreParagraph(paragraph: string, query: string): number {
      ...
      if (p.includes(q) && q.length > 10) score += 2;
      const len = Math.max(40, paragraph.length);
      score = score * (200 / len);
      return score;
    }

    function retrieve(topic: string | undefined, question: string) {
      const all = loadLLMDataset();
      const candidates = pickCandidateFiles(all, topic);
  const ranked: Array<{ file: LoadedDoc; startLine: number; endLine: number; text: string; score: number }> = [];
      for (const doc of candidates) {
        const paras = splitIntoParasWithLineRanges(doc.content);
        for (const para of paras) {
          const s = scoreParagraph(para.paragraph, question);
      if (s > 0.05) ranked.push({ file: doc, startLine: para.startLine, endLine: para.endLine, text: para.paragraph, score: s });
        }
      }
...
async function initTools(server: McpServer) {
    server.registerTool(
      ...
      if (!question || question.trim().length === 0) throw new Error("'question' is required");
